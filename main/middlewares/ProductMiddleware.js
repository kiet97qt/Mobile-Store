    ...
    return products
      ...
      .then((data) => {
        const newConfigs = new configs({
          ...
          flatform: [{ os: configuration.flatform[0].os, chipset: configuration.flatform[0].chipset }],
...
async function restore(data) {
  ...
  let configProduct = await getConfigProductDeleted(data.query.id).catch(() =>
    Promise.reject('Wrong ID_ConfigProduct!!'),
  );
  if (!configProduct) {
    return Promise.reject('Configration of this Product is not exist!!');
  }
